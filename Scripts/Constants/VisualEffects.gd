...
func play_critical_hit(sprite : Control, tween: Tween = null) -> Tween:
	if not tween:
		tween = create_tween()
	...
	# 1. Strong shake
	shake_node(sprite, crit_intensity, crit_duration, crit_loops, tween)
	
	# 2. Bright white/yellow flash instead of an alpha flicker
	flash_sprite(sprite, Color.GOLD, crit_duration, crit_loops, tween)
	
	return tween


func flash_sprite(sprite: CanvasItem, color: Color = Color.WHITE, total_duration: float = 0.15, loops: int = 2, tween: Tween = null) -> Tween:
	if not tween:
		tween = create_tween()
	...
	var single_step_duration = total_duration / (loops * 2.0)
	
	# Force the start of this loop sequence to run parallel to other effects
	tween.parallel() 
	
	for i in range(loops):
		# Chain these internal steps sequentially
		tween.tween_property(shader_material, "shader_parameter/flash_modifier", 1.0, single_step_duration)
		tween.tween_property(shader_material, "shader_parameter/flash_modifier", 0.0, single_step_duration)
	
	tween.tween_callback(func(): sprite.material = null)
	return tween

func shake_node(node: Control, intensity: Vector2 = Vector2(5, -5), total_duration: float = 0.15, loops: int = 3, tween: Tween = null) -> Tween:
	if not tween:
		tween = create_tween()
...
func flicker_node(node: CanvasItem, frequency: float = 0.04, loops: int = 5, tween: Tween = null) -> Tween:
	if not tween:
		tween = create_tween()
		
	tween.parallel() # Ensure it runs at the same time as shake and flash
	for i in range(loops):
